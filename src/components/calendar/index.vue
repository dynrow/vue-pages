<template>
    <div class="calendar">
                    ...
                    <svg width="20" height="20" viewBox="0 0 16 16" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                        <g class="transform-group">
                            <g transform="scale(0.015625, 0.015625)">
                                <path d="M671.968 912c-12.288 0-24.576-4.672-33.952-14.048L286.048 545.984c-18.752-18.72-18.752-49.12 0-67.872l351.968-352c18.752-18.752 49.12-18.752 67.872 0 18.752 18.72 18.752 49.12 0 67.872l-318.016 318.048 318.016 318.016c18.752 18.752 18.752 49.12 0 67.872C696.544 907.328 684.256 912 671.968 912z" fill="#555"></path>
                            </g>
                        </g>
                    </svg>
                </span>
                ...
                <div class="calendar-info" @click="changeYear">
                    ...
                    <div class="month">
                        <div class="month-inner" :style="{'top':-(this.month*30)+'px'}">
                            <span v-for="m in months" :key="m+'month'">{{year}} 年 {{m}}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <table cellpadding="5">
            <thead>
                <tr>
                    <td v-for="week in weeks" class="week" :key="week+'week'">{{week}}</td>
                </tr>
            </thead>
            <tbody>
                <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}" :key="k1+'tr'">
                    <td v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(k1,k2,$event)" :key="child.day+'day'">
        ...
        <div class="calendar-years" :class="{'show':yearsShow}">
            <span v-for="y in years" @click="selectYear(y)" :class="{'active':y==year}" :key="y+'year'">{{y}}</span>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        init() {
            //判断有没有传入默认时间
            //没有则设置当前客户端时间为默认时间
            let now = this.timestamp ? new Date(this.timestamp) : new Date(); 
            // 没有默认值
            this.year = now.getFullYear()
            this.month = now.getMonth()
            this.day = now.getDate()
            if (this.jcmday.length > 0) {
                if (this.range) {
                    this.year = parseInt(this.jcmday[0][0])
                    this.month = parseInt(this.jcmday[0][1]) - 1
                    this.day = parseInt(this.jcmday[0][2])
                    let year2 = parseInt(this.jcmday[1][0])
                    let month2 = parseInt(this.jcmday[1][1]) - 1
                    let day2 = parseInt(this.jcmday[1][2])
                    this.rangeBegin = [this.year, this.month, this.day]
                    this.rangeEnd = [year2, month2, day2]
                    // console.log(this.month,this.rangeBegin,this.rangeEnd)
                } else {
                    this.year = parseInt(this.jcmday[0]);
                    this.month = parseInt(this.jcmday[1]) - 1;
                    this.day = parseInt(this.jcmday[2]);
                }
            }
            this.render()
        },
        ...
        render() {
            let y = this.year; 
            let m = this.month;
            if(this.jcmday.length){
                y = parseInt(this.jcmday[0]);
                m = parseInt(this.jcmday[1]) - 1;
            }
            ...
            for (i = 1; i <= lastDateOfMonth; i++) {
                ...
                if (this.range) {
                    ...
                    // 匹配上次选中的日期
                    if (this.jcmday.length && this.jcmday[0] == this.year && this.jcmday[1] - 1 == this.month && this.jcmday[2] == i) {
                    ...
                    // 没有默认值的时候显示选中今天日期
                    else if (chkY == this.year && chkM == this.month && i == this.day) {
        ...
        prev(e) {
            ...
            this.$emit('selectMonth', [this.year, this.zero ? this.zeroPad(this.month + 1) : this.month + 1])
            this.$nextTick(()=>{
                _this.render()
            });
        },
        ...
        clickToday() {
            let _this = this;
            _this.yearsShow = false;
            let now = _this.timestamp ? new Date(_this.timestamp) : new Date(); 
            _this.year = now.getFullYear()
            _this.month = now.getMonth()
            _this.day = now.getDate()
            _this.$emit('select', [_this.year, _this.zero ? _this.zeroPad(_this.month + 1) : _this.month + 1, _this.zero ? _this.zeroPad(_this.day) : _this.day])
            _this.$nextTick(()=>{
                _this.render()
            });
        }
