 ...
 */
function MeScroll(mescrollDom, options) {
    this.scrollDom = mescrollDom; //MeScroll的滑动区域
...
MeScroll.prototype.extendDownScroll = function (optDown) {
    ...
    MeScroll.extend(optDown, {
        ...
        mustToTop: true, //是否必须滑动到顶部才能下拉;因为列表回弹效果(-webkit-overflow-scrolling:touch)是iOS专属样式,所以iOS默认false,其他为true;
...
MeScroll.prototype.showDownScroll = function () {
    ...
    this.downwarp.style.height = (this.optDown.offset / 2) + "px"; //调整下拉区域高度
}
    ...
    me.scrollDom.addEventListener("scroll", function () {
        ...
        if (!me.isUpScrolling && (!me.isDownScrolling || (me.isDownScrolling && me.optDown.isBoth))) {
            ...
            if (!me.optUp.isLock) {
                ...
                if (scrollTop > 0 && toBottom <= me.optUp.offset) {
...
MeScroll.prototype.resetUpScroll = function (isShowLoading) {
    if (this.optUp && this.optUp.use) {
        ...
        this.hideTopBtn();//隐藏返回顶部
        this.scrollDom.scrollTop = 0;//滚动条置顶
 ...
 */
MeScroll.prototype.endSuccess = function (dataSize, systime ,num) {
    //结束下拉刷新
    if (this.isDownScrolling) this.endDownScroll();
    //结束上拉加载
    if (this.optUp.use) {
        var pageNum = num === undefined ? this.optUp.page.num : num; //当前页码
