<template lang="html">
	<div class="yo-scroll" :class="{'down':(state===0),'up':(state==1),refresh:(state===2),touch:touching}" @touchstart="touchStart($event)" @touchmove="touchMove($event)" @touchend="touchEnd($event)">
		<section class="inner" :style="{ transform: 'translate3d(0, ' + top + 'px, 0)' }">
			...
			<footer class="load-more" v-show="infiniteCotr">
				<slot name="load-more">
					<span v-show="downFlag === false">上拉加载更多</span>
					<span v-show="downFlag === true">加载中……</span>
				</slot>
			</footer>
			<div class="nullData" v-show="options.noFlag">暂无更多</div>
		</section>
	</div>
</template>
	...
	export default {
		...
		methods: {
			touchStart(e) {
				...
				this.options.noFlag = false;
				this.infiniteCotr = true;
			},
			touchMove(e) {
                if(this.qualified()){
                    return;
                }
				if(!this.enableRefresh || this.options.noFlag || !this.touching) {
					return
				}
            ...
            //检测是否有滚动条,有才可以出发事件
            qualified(){
                if(this.$el.children && this.$el.children[0].offsetHeight < this.$el.offsetHeight){
                    return true;
                }
                return false;
            }
